...
for bs in "${bs_all[@]}"; do
    for lr in "${lr_all[@]}"; do
        for l in "${l_all[@]}"; do
            # imv
            `which python` src/train.py experiment=train_wavlm_16000/imv.yaml trainer=gpu +data.data.selected_labels=calltype data.batch_size=$bs model.optimizer.lr=$lr +layer_id=$l
            sleep 1
            `which python` src/train.py experiment=train_wavlm_16000/imv.yaml trainer=gpu +data.data.selected_labels=caller data.batch_size=$bs model.optimizer.lr=$lr +layer_id=$l
            sleep 1
            # # Alex
